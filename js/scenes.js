...
(function () {
  ...
  const world = {
    ...
    draw(ctx) {
      ...
      const me = this;
      const sprites = [{ baseY: py + 16, sprite: (c) => { if (me._onBoat) me.drawBoat(c, px, py); if (me._onRail) me.drawCart(c, px, py); if (me._dashT > 0) { c.save(); c.translate(px, py); c.rotate((1 - me._dashT / 0.2) * (me._dashDX >= 0 ? 1 : -1) * PI * 1.7); c.translate(-px, -py); G.drawSteve(c, px, py, face, walk, swing, heldK); c.restore(); } else G.drawSteve(c, px, py, face, walk, swing, heldK); if (me._dmgFlash > 0) { c.globalAlpha = Math.min(0.55, me._dmgFlash * 1.4); c.fillStyle = "#ff3030"; c.fillRect(px - 12, py - 27, 24, 44); c.globalAlpha = 1; } } }];
    ...
    drawHotbar(ctx) {
      ...
      for (let i = 0; i < G.HOTBAR; i++) {
        ...
        if (slot) {
          ...
          ctx.textAlign = "center"; ctx.font = G.f(34); ctx.fillStyle = "#fff";
    ...
    drawCraftCard(ctx, r, c) {
      ...
      ctx.font = G.f(38); ctx.fillText(out ? out.icon : "?", c.x + c.w / 2, c.y + 38);
    ...
    drawChest(ctx) {
      ...
      const cell = (r, c, sel) => {
        ...
        if (c) { const it = G.ITEMS[c.item]; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.font = G.f(28); ctx.fillStyle = "#fff"; ctx.fillText(it ? it.icon : "?", r.x + r.w / 2, r.y + r.h / 2 - 1); ctx.textAlign = "right"; ctx.font = G.f(14, "bold"); ctx.fillText("" + c.n, r.x + r.w - 5, r.y + r.h - 8); }
      };
    ...
    drawInv(ctx) {
      ...
      for (let i = 0; i < G.INV_SIZE; i++) {
        ...
        if (slot) {
          const it = G.ITEMS[slot.item];
          ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.font = G.f(34); ctx.fillStyle = "#fff";
          ctx.fillText(it ? it.icon : "?", r.x + r.w / 2, r.y + r.h / 2 - 1);
